...
Extending the descriptive visualization to the entire data set in form of a scatterplot (see \hyperref[F:2]{\color{blue}Figure 2, left panel}) again shows a first indication that the distance to Moscow - and, thus, Russia - may be affect a country's military expenditure, as the variance decreases if the distance increases. Yet, one can at the same time see the respective continent as well influencing the relationship. \textbf{!!!short part zu VDEM Panel- FOLGT NOCH!!!}.


\textbf{!!Hier das neue Model 1 mit VDEM!!}
\\

If we look at the second model

\begin{figure}[h]
\center
\label{F:2}
\includegraphics[scale=0.36]{PosteriorPlot_Distance_Model1.png}
\caption{}
\end{figure}
...
\item Limitations\item Implications 
\end{enumerate}
\clearpage
\section{Conclusion}
